...
The dataset's variables were evaluated for missing values, revealing that a consistent 4.25% of the data is missing across several environmental variables, including Td..C, Tw..C, X.rh, Tg..C, SR.W.m2, DP, Wind, and WBGT. This indicates potential issues with environmental data collection or processing. In contrast, other variables such as Race (categorical representation of locations like Boston, Chicago, etc.), Year, Sex, Flag, Age, and X.CR have complete data with no missing values. The absence of missing values in these demographic and categorical fields ensures their reliability for analysis. The uniformity of missing percentages across environmental variables might suggest a common source of data loss, which warrants further investigation.
...
data1 = data1 %>% mutate(Gender = ifelse(Gender == 0, "Female", "Male"))
...
data14 %>%
  ...
  tbl_summary(
    ...
    label = list(
      X.CR = "% off course record",
      Gender = "Gender of the runners",
      Age..yr. = "Age in years"
    )
  ) %>%
  modify_caption(caption = "Summary of Runner Characteristics by Marathon, 
                 N = {N}") %>%
  as_kable_extra(
    booktabs = TRUE,
    longtable = TRUE,
...
data14 = data14 %>% dplyr::rename( "NO2" = `Nitrogen dioxide (NO2)-1 HOUR`,
                                  "Ozone" = `Ozone-1 HOUR`,
...
kable(summary_table2, format = "latex", booktabs = TRUE, 
      caption = "Summary of Mean and Median Performance by Age Group and Gender (Ordered by Mean Performance") %>%
...
In the line plot below,we visualize the mean performance by age and gender. The interpretation of this plot is the same as of the above plots and tables. We can clearly see that the line is a "u-shape" line, meaning that children, teenagers and older athletes tend to run slower than the others, with male athletes running generally faster than female ones in all age groups.

```{r,fig.width=10, fig.height=8,fig.align='center', out.width= "70%"}
#Create a line plot to visualize the mean performance 
#by age, separated by gender.
## Mean performance plot
# Create a line plot to visualize the mean performance by Age, separated by gender.
# Summarize data to ensure one value per Age and Gender
# Summarize data and create plot with error bars

# Summarize data with mean performance and standard error
summary_stats1 <- data14 %>%
  group_by(Age..yr., Gender) %>%
  summarize(
    mean_performance = mean(performanceOpp, na.rm = TRUE),
    se = sd(performanceOpp, na.rm = TRUE) / sqrt(n())
  ) %>%
  ungroup()

# Create a line plot with error shading
mean_performance_plot <- summary_stats1 %>%
  ggplot(aes(
    x = Age..yr.,  # Continuous variable for x-axis
    y = mean_performance,  # Continuous variable for y-axis
    color = factor(Gender),
    fill = factor(Gender),  # Fill for shading
    group = Gender  # Ensures lines and shading are grouped by Gender
  )) +
  geom_ribbon(
    aes(ymin = mean_performance - se, ymax = mean_performance + se),
    alpha = 0.2,  # Transparency for the shaded area
    linetype = "blank"  # Remove border of the ribbon
  ) +
  geom_line(size = 1) +  # Add lines for each gender
  labs(
    title = "Mean Marathon Performance by Age and Gender with Shaded Error",
    x = "Age",
...
We also create histograms to explore the distribution of performance across age groups and gender. The histograms reveal that marathon participation is highest among individuals aged 20-69, with fewer participants in the youngest (14 and under) and oldest (70 and above) groups. Performance improves (lower times) in the 20-29, 30-39, and 40-49 age groups, where both men and women achieve their fastest times. However, starting from the 50-59 age group, performance declines significantly, with slower times and a rightward shift in the histograms. Male participants generally perform faster than females, particularly in younger groups, with an estimated 5-10% faster completion times, or around 500-1000 seconds difference. This gender gap diminishes slightly in older groups, indicating that age-related decline may impact men more.

Notably, performance variability increases with age. Younger groups (20-39) show more consistent times with sharp, concentrated peaks, while older groups (70-79 and above) display flatter distributions, highlighting greater variance. Older runners can take 2000-3000 seconds longer than those in their 20s, with some in the 70+ groups exceeding 30,000 seconds. These findings underscore that while age universally impacts performance, individual differences become more pronounced in older age groups.
...
# Add uncertainty (standard error) to the bar plot
mean_performance_plot <- data14 %>%
  group_by(Flag, Gender) %>%
  summarise(
    mean_performance = mean(performanceOpp, na.rm = TRUE),
    se = sd(performanceOpp, na.rm = TRUE) / sqrt(n())  # Calculate standard error
  ) %>%
  ggplot(aes(x = Flag, y = mean_performance, fill = factor(Gender))) +
  geom_bar(stat = "identity", position = position_dodge()) +  # Bar plot
  geom_errorbar(
    aes(ymin = mean_performance - se, ymax = mean_performance + se),  # Error bars
    position = position_dodge(width = 0.9),  # Align with bars
    width = 0.2  # Width of error bars
  ) +
  labs(
    title = "Mean Performance by WBGT Flag and Gender with Uncertainty",
    x = "WBGT Flag",
...
```{r, fig.width=6, fig.height=3,fig.align='center', out.width="70%"}
# Creating a new dataset for means
mean_data <- data14 %>%
  group_by(Wind, Gender) %>%
  summarise(mean_performance = mean(performanceOpp, na.rm = TRUE))

ggplot(mean_data, aes(x = Wind, y = mean_performance, color = as.factor(Gender))) +
  geom_point() +  # Add points for observed data
  geom_smooth(method = "loess", se = TRUE) +  # Add a smooth trend line with confidence interval
  labs(
    title = "Mean Performance by Wind and Gender",
    x = "Wind Speed",
    y = "Mean Performance (seconds)",
    color = "Gender"
  ) +
...
```{r,fig.width=5, fig.height=3,fig.align='center', out.width="70%"}
# Creating a new dataset for means
mean_data <- data14 %>%
...
ggplot(mean_data, aes(x = Wind, y = mean_performance, color = age_group)) +
  geom_point() +  # Add points for observed data
  #geom_line() +  # Add lines for mean performance
  geom_smooth(se = TRUE, method = "loess") +  # Add smooth trend lines with confidence intervals
  labs(
    title = "Mean Performance by Wind and Age Group",
    x = "Wind (Km/h)",
    y = "Mean Performance (seconds - Lower is faster)",
    color = "Age Group"
  ) +
...
We create a new dataset that combines our old dataset with a new one. The new dataset contains local pollutants measured in the AQI like PM2.5, Sulfur Dioxide, Nitrogen Dioxide and Ozone that we extracted with the `R` package [`RAQSAPI`](https://github.com/USEPA/RAQSAPI/).
...
merged_main <- merged_main %>%
  left_join(age_grade, by = c("Age", "Gender")) %>%
  mutate(age_graded_PerformaceOpp = Grade_Factor*PerformanceOpp)
```


```{r}
# Create a data frame with the performance and environmental variables
environmental_vars <- merged_main[, c(11, 6, 7, 8, 9, 12, 13, 14, 17)]
...
This study has several limitations that should be considered when interpreting the results. First, a portion of the data were missing, which may have affected the accuracy of the analysis. Additionally, the dataset only included a few major marathons, limiting the generalizability of the findings to other races or geographical regions. Furthermore, the data source itself may have limitations, such as inconsistencies in data collection methods, reliance on self-reported or event-reported metrics, and potential biases in how the data was recorded or processed. Individual factors such as fitness level, nutrition, and race strategy, which also likely influence performance, were not included in the analysis. Future research should consider incorporating these variables, addressing data source reliability, and collecting a more comprehensive dataset over a wider range of conditions.


# Data Privacy and Code Availability

The analysis dataset was obtained by Dr. Brett Romano Ely and Dr. Matthew Ely from the Department of Health Sciences at Providence College. The replication code can be found at <https://github.com/AristofanisR/Practical_Data_Analysis_Portfolio/tree/main/Project1>
